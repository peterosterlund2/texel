...
GameTree::getHeaders(std::map<std::string, std::string>& headers) {
    headers.emplace("Event", event);
    headers.emplace("Site",  site);
    headers.emplace("Date",  date);
    headers.emplace("Round", round);
    headers.emplace("White", white);
    headers.emplace("Black", black);
    for (size_t i = 0; i < tagPairs.size(); i++) {
        TagPair tp = tagPairs[i];
        headers.emplace(tp.tagName, tp.tagValue);
    }
}
