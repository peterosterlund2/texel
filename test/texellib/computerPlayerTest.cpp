...
ComputerPlayerTest::testGetCommand() {
    ...
    EXPECT_EQ("a5", result);     // Only one legal move

    pos = TextIO::readFEN("7k/5Q2/p5K1/8/8/8/8/8 b - - 100 80");
    result = cp.getCommand(pos, false, nullHist);
    EXPECT_EQ("draw 50", result);    // Should claim draw without making a move

    pos = TextIO::readFEN("3k4/1R6/R7/8/8/8/8/1K6 w - - 100 80");
    result = cp.getCommand(pos, false, nullHist);
    EXPECT_EQ("Ra8#", result);       // Can claim draw, but should not

    pos = TextIO::readFEN("8/1R5k/R7/8/8/8/B7/1K6 b - - 99 80");
    result = cp.getCommand(pos, false, nullHist);
    EXPECT_EQ("draw 50 Kh8", result);     // Should claim draw by 50-move rule
    ...
    result = cp.getCommand(pos, false, nullHist);
    EXPECT_EQ("Kxg8", result);
}

TEST(ComputerPlayerTest, testDrawRep) {
...
ComputerPlayerTest::testDrawRep() {
    ...
    EXPECT_EQ("Kh8", result); // Not valid to claim draw here
    ...
    EXPECT_EQ("draw rep Kh8", result);   // Can't win, but can claim draw.
    ...
    game.getHistory(hist);
    result = cp.getCommand(game.getPos(), false, hist);
    EXPECT_EQ("Ra7+", result);       // Ra7 is mate-in-two
    ...
    game.getHistory(hist);
    result = cp.getCommand(game.getPos(), false, hist);
    EXPECT_NE(result, "Ra7+"); // Ra7 now leads to a draw by repetition
}
