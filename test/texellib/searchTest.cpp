...
SearchTest::testSearch() {
    ...
    int score = sc->search(false, -mate0, mate0, ply, 2, Square(), MoveGen::inCheck(pos)) + ply;
    ...
    sc->setMinProbeDepth(100);
    score = sc->search(false, -mate0, mate0, ply, 4, Square(), MoveGen::inCheck(pos)) + ply;
    ...
    sc->init(pos, nullHist, 0);
    sc->setMinProbeDepth(100);
    score = sc->search(false, -mate0, mate0, ply, 5, Square(), MoveGen::inCheck(pos)) + ply;
    ...
    sc->init(pos, nullHist, 0);
    sc->setMinProbeDepth(100);
    score = sc->search(false, -mate0, mate0, ply, 2, Square(), MoveGen::inCheck(pos));
    ASSERT_EQ(0, score);             // Position is stale-mate

    pos = TextIO::readFEN("3kB3/8/1N1K4/8/8/8/8/8 w - - 0 1");
    sc->init(pos, nullHist, 0);
    sc->setMinProbeDepth(100);
    score = sc->search(false, -mate0, mate0, ply, 3, Square(), MoveGen::inCheck(pos));
...
SearchTest::testDraw50() {
    ...
    sc->maxTimeMillis = -1;
    sc->setMinProbeDepth(100);
    int score = sc->search(false, -mate0, mate0, ply, 2, Square(), MoveGen::inCheck(pos));
...
SearchTest::testDrawRep() {
    ...
    sc->maxTimeMillis = -1;
    sc->setMinProbeDepth(100);
    int score = sc->search(false, -mate0, mate0, ply, 3, Square(), MoveGen::inCheck(pos));
    EXPECT_EQ(0, score);
