...
#include "cute.h"
    ...
    public void testHaveDrawOffer() {
        ...
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());    // Draw offer does not imply draw
        ...
        ASSERT_EQUAL(true, res);
        ASSERT_EQUAL(Game::GameState.DRAW_AGREE, game.getGameState());    // Draw by agreement
        ...
        ASSERT_EQUAL(true, res);
        ASSERT_EQUAL(true, game.haveDrawOffer());
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());    // Can't redo draw accept
    ...
    public void testDraw50() {
        ...
        ASSERT_EQUAL(true, res);
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());    // Draw claim invalid
        res = game.processString("e4");
        ...
        ASSERT_EQUAL(Game::GameState.DRAW_50, game.getGameState());    // Draw claim valid
        ASSERT_EQUAL("Game over, draw by 50 move rule! [Nc3]", game.getGameStateString());

        game.processString(cmd);
        game.processString("draw 50 a6");
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());      // Pawn move resets counter
        ASSERT_EQUAL(Piece::WPAWN, game.pos.getPiece(Position::getSquare(0, 5))); // Move a6 made
        
        game.processString(cmd);
        game.processString("draw 50 O-O");
        ASSERT_EQUAL(Game::GameState.DRAW_50, game.getGameState());    // Castling doesn't reset counter
        
        game.processString(cmd);
        game.processString("draw 50 Kf2");
        ASSERT_EQUAL(Game::GameState.DRAW_50, game.getGameState());    // Loss of castling right doesn't reset counter
        
        game.processString(cmd);
        game.processString("draw 50 Ke3");
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());    // Ke3 is invalid
        ASSERT_EQUAL(true,game.pos.whiteMove);
        ...
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());  // 50 move counter reset.
        res = game.processString("draw accept");
        ASSERT_EQUAL(true, res);
        ASSERT_EQUAL(Game::GameState.DRAW_AGREE, game.getGameState()); // Can accept previous implicit offer
        
        cmd = "setpos 3k4/R7/3K4/8/8/8/8/8 w - - 99 78";
        game.processString(cmd);
        game.processString("Ra8");
        ASSERT_EQUAL(Game::GameState.WHITE_MATE, game.getGameState());
        game.processString("draw 50");
        ASSERT_EQUAL(Game::GameState.WHITE_MATE, game.getGameState()); // Can't claim draw when game over
        ASSERT_EQUAL(Game::GameState.ALIVE, game.drawState);
    }
    ...
    public void testDrawRep() {
        ...
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());    // Claim not valid, one more move needed
        game.processString("draw rep Nc6");
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());    // Claim not valid, wrong move claimed
        ...
        ASSERT_EQUAL(Game::GameState.DRAW_REP, game.getGameState());
    ...
    public void testResign() {
        ...
        ASSERT_EQUAL(Game::GameState.RESIGN_BLACK, game.getGameState());
        game.processString("undo");
        ASSERT_EQUAL(Game::GameState.ALIVE, game.getGameState());
        game.processString("f3");
        game.processString("e5");
        game.processString("resign");
        ASSERT_EQUAL(Game::GameState.RESIGN_WHITE, game.getGameState());
        game.processString("undo");
        game.processString("e5");
        game.processString("g4");
        game.processString("Qh4");
        ASSERT_EQUAL(Game::GameState.BLACK_MATE, game.getGameState());
        game.processString("resign");
        ASSERT_EQUAL(Game::GameState.BLACK_MATE, game.getGameState());   // Can't resign after game over
    }
    ...
    public void testGetGameState() {
        ...
        ASSERT_EQUAL(Game::GameState.BLACK_STALEMATE, game.getGameState());
    }
    ...
    public void testInsufficientMaterial() {
        ...
        ASSERT_EQUAL(Game::GameState.DRAW_NO_MATE, game.getGameState());
    ...
    public void testPerfT() {
        Game game = new Game(new HumanPlayer(), new HumanPlayer());
        game.processString("new");
        doTestPerfT(game.pos, 5, new U64[]{20,400,8902,197281,4865609,119060324,3195901860L,84998978956L});

        game.processString("setpos 8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - -");
        doTestPerfT(game.pos, 5, new U64[]{14, 191, 2812, 43238, 674624, 11030083, 178633661});
