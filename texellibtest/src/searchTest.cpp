...
SearchTest::testNegaScout() {
    ...
    Search sc(pos, nullHist, 0, st, pd, nullptr);
...
SearchTest::testDrawRep() {
    ...
    std::shared_ptr<Search> sc = std::make_shared<Search>(pos, nullHist, 0, st, pd, nullptr);
    sc->maxTimeMillis = -1;
    const int plyScale = SearchConst::plyScale;
    ...
    sc = std::make_shared<Search>(pos, nullHist, 0, st, pd, nullptr);
    sc->maxTimeMillis = -1;
