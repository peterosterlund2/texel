...
#include "cute.h"
    ...
    private bool testFENParseError(const std::string& fen) {
        bool wasError;
        wasError = false;
        try {
            TextIO::readFEN(fen);
...
//        printf("%s\n", aBrd.c_str());
        ASSERT_EQUAL(12, aBrd.length() - aBrd.replaceAll("\\*", "").length()); // 12 black pieces
        ASSERT_EQUAL(3, aBrd.length() - aBrd.replaceAll("\\*Q", " ").length()); // 3 black queens
        ASSERT_EQUAL(3, aBrd.length() - aBrd.replaceAll(" P", " ").length()); // 3 white pawns
    }
    ...
    public void testUciStringToMove() throws ChessParseError {
        ...
        ASSERT_EQUAL(new Move(12, 12+8*3, Piece::EMPTY), m);
        ...
        ASSERT_EQUAL(null, m);  // Invalid promotion piece
        m = TextIO::uciStringToMove("i1i3");
        ASSERT_EQUAL(null, m);  // Outside board
